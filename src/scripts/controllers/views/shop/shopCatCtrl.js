...
angular.module('app').controller('shopCatCtrl', ['rsFactory','cartService','$timeout','$stateParams', 
    function (rsFactory,cartService,$timeout,$stateParams) {
        ...
        rsFactory.categories.query(null, function(data){
            data.forEach(function(d){
                ...
                //Récuperation des articles de la categorie sélectionnée
                rsFactory.articlesByCat($stateParams.catId).then(function(data){
                    vm.articles = data;
                    delete vm.articles.$promise;
                    delete vm.articles.$resolved;
                    vm.newProducts = getNewProducts(vm.articles,"New products");
                    vm.updateStyleCatSelected($stateParams.catId,null); 
                });
